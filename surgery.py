...
# python surgery.py splash --weights=D:\标注\413-would\新建文件夹\mrcnn\logs\mask_rcnn_surgery_0060.h5 --image=D:\标注\413-would\新建文件夹\mrcnn\test\2020-04-13T22.07.05.jpg
...
class SurgeryDataset(utils.Dataset):
    def load_VIA(self, dataset_dir, subset, hc=False):
        ...
        self.add_class("surgery", 1, "wound")
        ...
        for a in annotations:
            ...
            polygons = [r['shape_attributes'] for r in a['regions']]   #要修改  json的格式变了
    ...
    #self.image_info为数组，每个值存放的是每张图： ：           
    #           "surgery",                          
                # image_id=a['filename'],  # use file name as a unique image id是个字符串
                # path=image_path,                  是个字符串
                # width=width, height=height,
                # polygons=polygons,                shape_attributes是个字典                    polygons时字典组成的数组
                # names=names                       region_attributes是个字典                   names时字典组成的数组  每个字典的序号与上面polygons对应
    # 参数image_id为整数，是个序号
